...
RETRIEVAL_REQUESTS = Counter(
    ...
    "Knowledge retrieval requests count",
    ["knowledge_id", "method"]
)
...
async def knowledge_retrieval(
    ...
    """处理知识库检索请求"""
    # 记录请求
    current_knowledge_id = knowledge_id if request.method == "GET" else data.knowledge_id
    RETRIEVAL_REQUESTS.labels(
        knowledge_id=current_knowledge_id,
        method=request.method
    ).inc()
    
    with RETRIEVAL_LATENCY.labels(knowledge_id=current_knowledge_id).time():
        # 验证认证信息
